...
export function RecentActivityCard(): JSX.Element {
  ...
  const getEventIcon = (event: string) => {
    switch (event) {
      ...
      case 'tab-closed': return '✖️';
      case 'tab-activated': return '👁️';
      case 'navigation': return '🧭';
      case 'rule-match': return '⚡';
      default: return '📄';
    }
  };
  ...
  const getEventLabel = (event: string) => {
    switch (event) {
      case 'tab-created': return 'Tab opened';
      case 'tab-closed': return 'Tab closed';
      case 'tab-activated': return 'Tab activated';
      case 'navigation': return 'Navigation';
      case 'rule-match': return 'Rule matched';
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
      ...
      <div className="space-y-3">
        {recentActivity.slice(0, 8).map((activity) => {
          const container = containers.find(c => c.cookieStoreId === activity.containerId);
