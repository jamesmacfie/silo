...
describe('ContainerSelector', () => {
  ...
  it('renders containers and handles select', async () => {
    (browser.runtime.sendMessage as jest.Mock).mockResolvedValue({
      success: true,
      data: [
        { id: '1', name: 'Work', icon: 'briefcase', color: 'blue', cookieStoreId: 'fx-1', created: 1, modified: 1, temporary: false, syncEnabled: true },
        { id: '2', name: 'Personal', icon: 'gift', color: 'red', cookieStoreId: 'fx-2', created: 1, modified: 1, temporary: false, syncEnabled: true },
      ],
    });
    ...
    expect(onSelect).toHaveBeenCalledWith(expect.objectContaining({ name: 'Personal' }));
  });
});
