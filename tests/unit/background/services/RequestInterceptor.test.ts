...
describe("RequestInterceptor", () => {
  ...
  beforeEach(() => {
    ...
    mockBookmarkIntegration = bookmarkIntegration as jest.Mocked<
      typeof bookmarkIntegration
    >
  ...
  describe("register", () => {
    ...
    it("should throw error if registration fails completely", async () => {
      ...
      await expect(requestInterceptor.register()).rejects.toThrow(
        "Failed to register",
      )
    })
  })
  ...
  describe("unregister", () => {
    ...
    it("should handle errors during unregistration", async () => {
      ;(
        browser.webRequest.onBeforeRequest.removeListener as jest.Mock
      ).mockImplementation(() => {
  ...
  describe("handleRequest", () => {
    ...
    it("should skip non-main frame requests", async () => {
      ...
      const result = await (requestInterceptor as any).handleRequest(
        subFrameDetails,
      )
  ...
  describe("handleRedirect", () => {
    ...
    it("should handle tab creation errors", async () => {
      ;(browser.tabs.create as jest.Mock).mockRejectedValue(
        new Error("Failed to create tab"),
      )
  ...
  describe("showRestrictionNotification", () => {
    ...
    it("should handle unknown container", async () => {
      ...
      expect(browser.notifications.create).toHaveBeenCalledWith(
        expect.objectContaining({
          message:
            'example.com can only be opened in "Unknown Container" container.',
        }),
      )
    })
    ...
    it("should handle notification creation errors", async () => {
      mockContainerManager.getAll = jest.fn().mockResolvedValue([mockContainer])
      ;(browser.notifications.create as jest.Mock).mockRejectedValue(
        new Error("Permission denied"),
      )

      // Should not throw
      await expect(
        (requestInterceptor as any).showRestrictionNotification(
          "https://example.com",
          "firefox-container-1",
        ),
  ...
  describe("showExcludeNotification", () => {
    it("should create exclude notification", async () => {
      await (requestInterceptor as any).showExcludeNotification(
        "https://example.com",
      )
    ...
    it("should handle notification creation errors", async () => {
      ...
      // Should not throw
      await expect(
        (requestInterceptor as any).showExcludeNotification(
          "https://example.com",
        ),
      ).resolves.not.toThrow()
    })
  ...
  describe("handleTabUpdate", () => {
    ...
    it("should handle errors gracefully", async () => {
      mockRulesEngine.evaluate = jest
        .fn()
        .mockRejectedValue(new Error("Evaluation failed"))

      await expect(
        (requestInterceptor as any).handleTabUpdate(
          1,
          { url: "https://example.com" },
          mockTab,
        ),
  ...
  describe("handleTabContainerUpdate", () => {
    it("should create new tab in target container and close old tab", async () => {
      ...
      await (requestInterceptor as any).handleTabContainerUpdate(
        1,
        "https://example.com",
        evaluation,
      )
    ...
    it("should handle tab operation errors", async () => {
      ...
      await expect(
        (requestInterceptor as any).handleTabContainerUpdate(
          1,
          "https://example.com",
          { action: "redirect" },
        ),
  ...
  describe("handleTabCreated", () => {
    ...
    it("should handle stats recording errors", async () => {
      mockStorageService.recordStat = jest
        .fn()
  ...
  describe("handleTabRemoved", () => {
    it("should record close stats and cleanup temporary containers", async () => {
      ...
      await (requestInterceptor as any).handleTabCreated({
        ...mockTab,
        cookieStoreId: "firefox-container-1",
      })
  ...
  describe("shouldIntercept", () => {
    it("should intercept HTTP and HTTPS URLs", () => {
      expect(requestInterceptor.shouldIntercept("https://example.com")).toBe(
        true,
      )
      expect(requestInterceptor.shouldIntercept("http://example.com")).toBe(
        true,
      )
    })
    ...
    it("should not intercept browser internal URLs", () => {
      ...
      expect(requestInterceptor.shouldIntercept("moz-extension://abc")).toBe(
        false,
      )
      expect(requestInterceptor.shouldIntercept("file:///path/to/file")).toBe(
        false,
      )
      expect(requestInterceptor.shouldIntercept("javascript:alert(1)")).toBe(
        false,
      )
      expect(requestInterceptor.shouldIntercept("data:text/html,test")).toBe(
        false,
      )
      expect(requestInterceptor.shouldIntercept("chrome://settings")).toBe(
        false,
      )
      expect(requestInterceptor.shouldIntercept("chrome-extension://abc")).toBe(
        false,
      )
    })
    ...
    it("should handle edge cases", () => {
      ...
      expect(requestInterceptor.shouldIntercept("ftp://example.com")).toBe(
        false,
      )
    })
  })
