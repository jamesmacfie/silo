        ...
        try {
          ...
          set(state => ({
            containers: state.containers.map(c => 
              c.cookieStoreId === id ? { ...c, ...updates, modified: Date.now() } : c
            )
          }));
        } catch (error) {
          set({ error: error instanceof Error ? error.message : 'Unknown error' });
...
export const useContainers = () => useContainerStore(state => state.containers);
export const useContainerActions = () => useContainerStore(state => state.actions);
export const useContainerLoading = () => useContainerStore(state => state.loading);
