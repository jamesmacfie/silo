...
const useStatsStore = create<StatsState>((set, get) => ({
  ...
  load: async () => {
    ...
    try {
      const response = await messagingService.sendMessage(MESSAGE_TYPES.GET_STATS);
      
      if (response.success) {
        set({ 
          containerStats: (response.data as Record<string, ContainerStats>) || {},
          loading: false 
        });
      } else {
  ...
  loadGlobalStats: async () => {
    try {
      const response = await messagingService.sendMessage(MESSAGE_TYPES.GET_GLOBAL_STATS);
      ...
      set({ error: error instanceof Error ? error.message : 'Failed to load global stats' });
    }
  },

  loadActiveTabs: async () => {
    try {
      const response = await messagingService.sendMessage(MESSAGE_TYPES.GET_ACTIVE_TABS);
      ...
      set({ error: error instanceof Error ? error.message : 'Failed to load active tabs' });
    }
  },

  loadRecentActivity: async () => {
    try {
      const response = await messagingService.sendMessage(MESSAGE_TYPES.GET_RECENT_ACTIVITY);
      ...
      set({ error: error instanceof Error ? error.message : 'Failed to load recent activity' });
    }
  },

  loadDailyStats: async (days = 7) => {
    try {
      const response = await messagingService.sendMessage(MESSAGE_TYPES.GET_DAILY_STATS, { days });
      ...
      set({ error: error instanceof Error ? error.message : 'Failed to load daily stats' });
    }
  },

  loadTrends: async (days = 30) => {
    try {
      const response = await messagingService.sendMessage(MESSAGE_TYPES.GET_CONTAINER_TRENDS, { days });
  ...
  reset: async () => {
    try {
      const response = await messagingService.sendMessage(MESSAGE_TYPES.RESET_STATS);
      ...
      set({ error: error instanceof Error ? error.message : 'Failed to reset stats' });
    }
  },
...
export const useRecentActivity = () => useStatsStore(state => state.recentActivity);
