...
export class RequestInterceptor {
  ...
  async unregister(): Promise<void> {
    ...
    try {
      browser.webRequest.onBeforeRequest.removeListener(this.handleRequest.bind(this));
    ...
    try {
      ...
      switch (evaluation.action) {
        case 'redirect':
          ...
          return await this.handleRedirect(urlToEvaluate, tabId, evaluation.containerId, evaluation);

        case 'exclude':
          return await this.handleExclude(urlToEvaluate, tabId, evaluation, currentContainer);
        ...
        default:
          ...
          if (processed.containerId && processed.containerId !== currentContainer) {
            return await this.handleRedirect(urlToEvaluate, tabId, processed.containerId, { action: 'redirect', containerId: processed.containerId, reason: 'Bookmark silo param' });
          }
          // Count matches even when no redirect is needed (already in correct container or no-op include)
          try {
            if (evaluation.rule) {
              const containerForMatch = evaluation.rule.ruleType === 'exclude'
                ? (currentContainer && currentContainer !== 'firefox-default' ? currentContainer : undefined)
  ...
  private async handleRedirect(
    ...
    try {
      ...
      try {
        if (evaluation.rule) { await this.storage.recordStat(targetContainer, 'match'); }
      } catch { /* ignore stats errors */ }
  ...
  private async showRestrictionNotification(url: string, containerId: string): Promise<void> {
    try {
  ...
  private async handleTabUpdate(
    ...
    try {
      ...
      if (evaluation.action === 'open' && processed.containerId && processed.containerId !== currentContainer) {
        await this.handleTabContainerUpdate(tabId, urlToEvaluate, { action: 'redirect', containerId: processed.containerId });
        return;
      }
      ...
      this.log.error('Error handling tab update', { tabId, url: tab.url, error });
    }
  }

  private async handleTabContainerUpdate(tabId: number, url: string, evaluation: EvaluationResult): Promise<void> {
    try {
      const useDefault = evaluation.action === 'exclude' || evaluation.containerId === 'firefox-default';
      ...
      const newTab = await browser.tabs.create(useDefault ? { ...createProps, cookieStoreId: 'firefox-default' } : createProps);

      await browser.tabs.remove(tabId);

      const targetContainer = useDefault ? 'firefox-default' : evaluation.containerId;
  ...
  private async handleTabRemoved(tabId: number, _removeInfo: browser.Tabs.OnRemovedRemoveInfoType): Promise<void> {
    try {
  ...
  shouldIntercept(url: string): boolean {
    ...
    const IGNORED_URLS_REGEX = /^(about|moz-extension|file|javascript|data|chrome|chrome-extension):/;
