...
class StatsService {
  ...
  private async flushBatchBuffer(): Promise<void> {
    ...
    try {
      ...
      this.batchBuffer.unshift(
        ...events.slice(0, this.MAX_BATCH_SIZE - this.batchBuffer.length),
      )
    }
  }
  ...
  private async updateContainerStats(
    containerId: string,
    events: StatEvent[],
  ): Promise<void> {
    ...
    for (const event of events) {
      ...
      await storageService.recordStat(
        containerId,
        this.mapEventToLegacy(event.event),
      )
    ...
    current.activeTabCount = Math.max(
      0,
      (current.activeTabCount || 0) + activeTabDelta,
    )
    current.lastUsed = Math.max(
      current.lastUsed || 0,
  ...
  private mapEventToLegacy(
    event: string,
  ): "open" | "match" | "close" | "touch" {
    switch (event) {
      case "tab-created":
        return "open"
      case "tab-closed":
  ...
  private async updateGlobalStats(events: StatEvent[]): Promise<void> {
    ...
    const updated: GlobalStats = {
      ...
      totalTabsEverOpened:
        current.totalTabsEverOpened + tabCreatedEvents.length,
    ...
    const stats = await storageService.getStats()
    const mostUsed = Object.entries(stats).sort(
      ([, a], [, b]) => (b.tabsOpened || 0) - (a.tabsOpened || 0),
    )[0]
    if (mostUsed) {
  ...
  async getDailyStats(days: number = 7): Promise<DailyStats[]> {
    try {
      const dailyStats =
        (await storageService.get<DailyStats[]>(STORAGE_KEYS.DAILY_STATS)) || []
  ...
  async getContainerTrends(days: number = 30): Promise<TrendData> {
    try {
      ...
      const containerUsage = containers.map((container) => {
        const containerStats = dailyStats.filter(
          (s) => s.containerId === container.cookieStoreId,
        )
      ...
      const totalTrend = dailyStats.reduce(
        (acc, stat) => {
  ...
  async getGlobalStats(): Promise<GlobalStats> {
    try {
      const stats = await storageService.get<GlobalStats>(
  ...
  async getRecentActivity(): Promise<ActivityEvent[]> {
    try {
      return (
        (await storageService.get<ActivityEvent[]>(
