...
describe("PopupApp", () => {
  ...
  beforeEach(async () => {
    ...
    ;(browser.contextualIdentities.get as jest.Mock).mockImplementation(
      (cookieStoreId) => {
  ...
  describe("initial render", () => {
    ...
    it("should show current tab information", async () => {
      ...
      const contextDiv = screen.getByTestId
        ? screen.queryByTestId("contextInfo")
        : screen.getByText(/Tab:/i).parentElement
  ...
  describe("action buttons", () => {
    ...
    it("should have Bookmark button", async () => {
      ...
      const bookmarkButton = screen.getByTitle(
        "Create a bookmark for this page in the current container",
      )
  ...
  describe("container operations", () => {
    it("should open current URL in selected container", async () => {
      ...
      await waitFor(
        () => {
          ...
          const openInContainerCall = calls.find(
            (call) => call[0]?.type === "OPEN_IN_CONTAINER",
          )
    ...
    it("should open URL without container when No Container is selected", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("button", { name: /open in container/i }),
        ).toBeInTheDocument()
      })
    ...
    it("should create temporary container", async () => {
      ...
      await waitFor(
        () => {
          ...
          expect(
            screen.getByText("Temporary container created"),
          ).toBeInTheDocument()
        },
        { timeout: 1000 },
      )
    ...
    it("should add domain rule for current tab", async () => {
      ...
      await waitFor(() => {
        const calls = (browser.runtime.sendMessage as jest.Mock).mock.calls
        const createRuleCall = calls.find(
          (call) => call[0]?.type === "CREATE_RULE",
        )
  ...
  describe("error handling", () => {
    ...
    it("should handle container creation errors", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText(/failed to create temp: creation failed/i),
          ).toBeInTheDocument()
        },
        { timeout: 1000 },
      )
    ...
    it("should handle bookmark creation errors", async () => {
      const user = userEvent.setup({ delay: null })
      ;(browser.bookmarks.create as jest.Mock).mockRejectedValue(
        new Error("Bookmark failed"),
      )
