...
export function PopupApp(_props: Props): JSX.Element {
  ...
  const [selectedCookieStoreId, setSelectedCookieStoreId] = React.useState<string>('firefox-default');
  const [currentHost, setCurrentHost] = React.useState<string>('—');
  const [currentContainerName, setCurrentContainerName] = React.useState<string>('—');
  const [status, setStatus] = React.useState<string>('');

  React.useEffect(() => {
    if (containers.length && !containers.find(c => c.cookieStoreId === selectedCookieStoreId)) {
      ...
      if (tab?.url) {
        ...
        if (cookieStoreId && browser.contextualIdentities?.get) {
          try {
            ...
            setCurrentContainerName(ci?.name || (cookieStoreId === 'firefox-default' ? 'No Container' : '—'));
          } catch {
            setCurrentContainerName(cookieStoreId === 'firefox-default' ? 'No Container' : '—');
          }
        } else {
          setCurrentContainerName(cookieStoreId === 'firefox-default' ? 'No Container' : '—');
        }
  ...
  const onManageContainers = React.useCallback(async () => {
    ...
    setTimeout(() => { window.close(); return; }, 50);
    return;
  }, []);
  ...
  return (
    <div className="popup">
      ...
      <select
        ...
        onChange={(e) => { setSelectedCookieStoreId(e.target.value); return; }}
      >
        {containers.map((c) => (
          <option key={c.cookieStoreId} value={c.cookieStoreId}>{c.name}</option>
        ))}
      </select>
      ...
      <div className="row mt-2">
        ...
        <button className="ghost small" title="Create temporary container" type="button" onClick={() => { createTemporaryContainer(); return; }}>
          + Temp
        </button>
        <button className="ghost small" title="Create a bookmark for this page in the current container" type="button" onClick={() => { bookmarkCurrentTab(); return; }}>
          ⭐︎ Bookmark
        </button>
      </div>
      ...
      <div className="footerLink">
        <a
          className="link"
          href="/options.html"
          onClick={(e) => { e.preventDefault(); onManageContainers(); return; }}
        >
          Manage Containers →
        </a>
      </div>
