    ...
    initialize: async () => {
      try {
        ...
        await Promise.all([
          ...
          useBookmarkStore.getState().actions.loadAssociations(),
          useBookmarkStore.getState().actions.loadBookmarksTree(),
        ]);
        
        set({ isInitialized: true });
      } catch (error) {
        set({ 
          initializationError: error instanceof Error ? error.message : 'Failed to initialize app',
          isInitialized: false 
        });
      }
    },
      ...
      (containers, prevContainers) => {
        ...
        if (deletedIds.length > 0) {
          ...
          rulesToDelete.forEach(ruleId => {
            useRuleStore.getState().actions.delete(ruleId).catch(() => {
              // Silently fail - the rule might already be deleted
            });
          });
          
          // Clean up bookmark associations for deleted containers
          const associations = useBookmarkStore.getState().associations;
          const associationsToDelete = associations
            .filter(a => deletedIds.includes(a.containerId))
            .map(a => a.bookmarkId);
          
          associationsToDelete.forEach(bookmarkId => {
            useBookmarkStore.getState().actions.removeAssociation(bookmarkId).catch(() => {
              // Silently fail
            });
          });
        }
...
export const useGlobalErrors = () => {
  ...
  const errors = [
    ...
    initializationError && { type: 'initialization', message: initializationError },
  ].filter(Boolean);
...
export const useGlobalLoading = () => {
  ...
  return {
    ...
    bookmarks: bookmarkLoading.associations || bookmarkLoading.tree,
    isInitializing: !isInitialized,
    isLoading: containerLoading || ruleLoading || themeLoading || preferencesLoading || bookmarkLoading.associations || bookmarkLoading.tree,
  };
};