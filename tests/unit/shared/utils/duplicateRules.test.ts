import { findDuplicateRules, getDuplicateCount, suggestRulesToKeep } from '@/shared/utils/duplicateRules';
...
describe('duplicateRules', () => {
  ...
  describe('suggestRulesToKeep', () => {
    it('should suggest keeping rule with highest priority', () => {
      const rules = [
        createMockRule({ id: '1', pattern: 'example.com', priority: 1, created: 1000 }),
        createMockRule({ id: '2', pattern: 'example.com', priority: 5, created: 2000 }),
        createMockRule({ id: '3', pattern: 'example.com', priority: 3, created: 3000 }),
      ];
      ...
      expect(remove.map(r => r.id)).toEqual(expect.arrayContaining(['1', '3']));
    });

    it('should suggest keeping most recently modified when priorities are equal', () => {
      const rules = [
        createMockRule({ id: '1', pattern: 'example.com', priority: 1, modified: 1000 }),
        createMockRule({ id: '2', pattern: 'example.com', priority: 1, modified: 3000 }),
        createMockRule({ id: '3', pattern: 'example.com', priority: 1, modified: 2000 }),
      ];
