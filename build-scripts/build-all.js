...
async function buildAll() {
  ...
  try {
    ...
    execSync('npm run build:popup && npm run build:popup:css', { stdio: 'inherit', cwd: rootDir });
    execSync('npm run build:options && npm run build:options:css', { stdio: 'inherit', cwd: rootDir });
    ...
    execSync(`extension build ${browser}`.trim(), { stdio: 'inherit', cwd: rootDir });

    // Cleanup - remove files from root
    console.log('🧹 Cleaning up...');
    const allFilesToClean = [...tempFilesToCopy, ...htmlFilesToCopy.map(h => h.dest)];
