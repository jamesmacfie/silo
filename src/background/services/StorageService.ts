...
export class StorageService {
  ...
  private async setupDefaults(): Promise<void> {
    await browser.storage.local.set({
      ...
      [STORAGE_KEYS.TEMPLATES]: [
        { name: 'Work', color: 'blue', icon: 'briefcase', metadata: { lifetime: 'permanent', categories: ['Work'] } },
        { name: 'Personal', color: 'purple', icon: 'gift', metadata: { lifetime: 'permanent', categories: ['Personal'] } },
        { name: 'Banking', color: 'green', icon: 'dollar', metadata: { lifetime: 'untilLastTab', categories: ['Personal'] } },
        { name: 'Social', color: 'pink', icon: 'fruit', metadata: { lifetime: 'permanent' } },
        { name: 'Dev', color: 'orange', icon: 'tree', metadata: { lifetime: 'permanent' } },
  ...
  async backup(): Promise<BackupData> {
    const [containers, rules, preferences, bookmarksRaw, categories, stats] = await Promise.all([
      this.getContainers(),
      this.getRules(),
      this.getPreferences(),
  ...
  async restore(backup: BackupData): Promise<void> {
    ...
    if (!backup.version || backup.version !== this.CURRENT_VERSION) {
      throw new Error(`Backup version ${backup.version || 'unknown'} is not compatible with current version ${this.CURRENT_VERSION}`);
    }
  ...
  private validateRules(rules: Rule[]): ValidationResult {
    ...
    rules.forEach((rule, index) => {
      ...
      if ((rule.ruleType === 'include' || rule.ruleType === 'restrict') && !rule.containerId) {
        errors.push(`Missing containerId at index ${index} for ${rule.ruleType.toUpperCase()} rule`);
      }
  ...
  async getStats(): Promise<Record<string, ContainerStats>> {
    const stats = await this.get<Record<string, ContainerStats>>(STORAGE_KEYS.STATS);
    return stats || {};
  }
  ...
  async recordStat(cookieStoreId: string, event: 'open' | 'match' | 'close' | 'touch'): Promise<void> {
    ...
    const current = stats[cookieStoreId] || {
  ...
  async saveTemplate(template: ContainerTemplate): Promise<void> {
    ...
    const idx = list.findIndex((t: ContainerTemplate) => t.name === template.name);
    if (idx !== -1) {
      list[idx] = template;
    } else {
      list.push(template);
  ...
  async setBookmarkAssociations(associations: BookmarkAssociation[]): Promise<void> {
    await this.set(STORAGE_KEYS.BOOKMARKS, associations);
  }

  async addBookmarkAssociation(association: BookmarkAssociation): Promise<void> {
    const list = await this.getBookmarkAssociations();
    const existingIndex = list.findIndex(a => a.bookmarkId === association.bookmarkId);
    if (existingIndex !== -1) {
  ...
  async getBookmarkAssociation(bookmarkId: string): Promise<BookmarkAssociation | null> {
    const list = await this.getBookmarkAssociations();
