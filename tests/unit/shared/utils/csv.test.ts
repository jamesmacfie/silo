import { parseCSV, exportToCSV as generateCSV, validateCSVRow, type CSVValidationResult } from '@/shared/utils/csv';
...
describe('CSV Utilities', () => {
  ...
  describe('generateCSV', () => {
    it('should generate CSV with header', () => {
      ...
      expect(result).toContain('# Format: pattern, container_name, match_type, rule_type, priority, enabled, description');
    });
...
bitbucket.com,Personal`;
      ...
      const result = parseCSV(csv, mockContainers, { createMissingContainers: true });
...
bitbucket.com,Personal`;
  ...
  describe('validateCSVRow', () => {
    ...
    it('should validate container existence', () => {
      const result = validateCSVRow(['github.com', 'NonExistent'], mockContainers);
    ...
    it('should allow empty container for exclude rules', () => {
      const result = validateCSVRow(['github.com', '', 'domain', 'exclude'], mockContainers);
    ...
    it('should validate match type', () => {
      ...
      for (const type of validTypes) {
        const result = validateCSVRow(['github.com', 'Work', type], mockContainers);
        expect(result.isValid).toBe(true);
      }

      const result = validateCSVRow(['github.com', 'Work', invalidType], mockContainers);
      expect(result.isValid).toBe(false);
    ...
    it('should validate rule type', () => {
      ...
      for (const type of validTypes) {
        const result = validateCSVRow(['github.com', 'Work', 'domain', type], mockContainers);
        expect(result.isValid).toBe(true);
      }

      const result = validateCSVRow(['github.com', 'Work', 'domain', invalidType], mockContainers);
      expect(result.isValid).toBe(false);
    ...
    it('should validate enabled flag', () => {
      ...
      for (const flag of validFlags) {
        const result = validateCSVRow(['github.com', 'Work', 'domain', 'include', '1', flag], mockContainers);
        expect(result.isValid).toBe(true);
      }
    ...
    it('should handle too many fields', () => {
      const result = validateCSVRow([
        'github.com', 'Work', 'domain', 'include', '1', 'true', 'extra', 'fields'
      ], mockContainers);
