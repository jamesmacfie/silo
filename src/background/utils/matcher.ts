...
export class URLMatcher {
  ...
  static getInstance(): URLMatcher {
    if (!this.instance) {
      this.instance = new URLMatcher();
    }
    return this.instance;
  }

  match(url: string, pattern: string, type: MatchType): boolean {
    try {
      ...
      switch (effectiveType) {
        case MatchType.EXACT:
          return this.matchExact(url, effectivePattern);
        case MatchType.DOMAIN:
          return this.matchDomain(url, effectivePattern);
        case MatchType.GLOB:
          return this.matchGlob(url, effectivePattern);
        case MatchType.REGEX:
          return this.matchRegex(url, effectivePattern);
        default:
  ...
  private matchDomain(url: string, pattern: string): boolean {
    const { hostname: urlDomain, pathname: urlPathname } = this.parse(url);
    ...
    let patternHostname = this.extractDomain(pattern).toLowerCase();
    let patternPathname = '';
    try {
...
export default URLMatcher.getInstance();