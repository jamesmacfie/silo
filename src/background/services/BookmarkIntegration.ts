...
export class BookmarkIntegration {
    ...
    try {
      ...
      const walk = (nodes: browser.Bookmarks.BookmarkTreeNode[]) => {
        for (const node of nodes) {
          if (node.url) {
            const { containerHint, cleanUrl } = this.extractContainerFromUrl(
              node.url,
            )
            if (containerHint) {
      ...
      if (associations.length > 0) {
        ...
        const resolved = await Promise.all(
          associations.map(async (a) => ({
            ...a,
            containerId:
              (await this.resolveToCookieStoreId(a.containerId)) ||
              a.containerId,
          })),
        )
        await this.storage.setBookmarkAssociations(resolved)
        this.log.info("Synced bookmark associations from bookmarks tree", {
  ...
  private extractContainerFromUrl(url: string): {
    cleanUrl: string
  ...
  async resolveToCookieStoreId(value: string): Promise<string | undefined> {
    ...
    const byCookieStore = all.find(
      (c) => c.cookieStoreId.toLowerCase() === normalized,
    )
    if (byCookieStore) {
