...
export class MessagingService {
  ...
  async getContainers(): Promise<Container[]> {
    const response = await this.sendMessage<Container[]>(
  ...
  async createRule(request: CreateRuleRequest): Promise<Rule> {
    const response = await this.sendMessage<Rule>(
  ...
  async getPreferences(): Promise<Preferences> {
    const response = await this.sendMessage<Preferences>(
  ...
  async backupData(): Promise<BackupData> {
    const response = await this.sendMessage<BackupData>(
  ...
  async addBookmarkAssociation(payload: {
    bookmarkId: string
    containerId: string
    url: string
    autoOpen?: boolean
  ...
  async removeBookmarkAssociation(bookmarkId: string) {
    await this.sendMessage(MESSAGE_TYPES.REMOVE_BOOKMARK_ASSOCIATION, {
      bookmarkId,
    })
  }

  async processBookmarkUrl(
    url: string,
  ): Promise<{ cleanUrl: string; containerId?: string }> {
    const response = await this.sendMessage(
      MESSAGE_TYPES.PROCESS_BOOKMARK_URL,
      { url },
    )
    return (response.data || { cleanUrl: url }) as {
      cleanUrl: string
      containerId?: string
    }
  }
