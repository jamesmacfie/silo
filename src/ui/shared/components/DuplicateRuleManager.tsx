    ...
    <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-yellow-50 dark:bg-yellow-900/20">
      <div className="mb-3">
        ...
        <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">
          Type: {group.ruleType} | Match: {group.matchType} | Container:{" "}
          {getContainerName(group.containerId)}
        </div>
      </div>
...
export function DuplicateRuleManager({
  rules,
  containers,
  onDeleteRule,
}: DuplicateRuleManagerProps) {
  const [selectedForRemoval, setSelectedForRemoval] = React.useState<
    Set<string>
  >(new Set())
  const [isRemoving, setIsRemoving] = React.useState(false)

  const duplicateGroups = React.useMemo(
    () => findDuplicateRules(rules),
    [rules],
  )
  ...
  const handleRemoveSelected = React.useCallback(async () => {
    ...
    try {
      ...
      if (errors.length === 0) {
        ...
        alert(
          `Successfully removed ${successCount} rule(s). Failed to remove ${errors.length} rule(s). Please try again for the remaining rules.`,
        )
      } else {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <span className="text-sm font-medium text-yellow-800 dark:text-yellow-200">
              Found {duplicateCount} duplicate rule(s) in{" "}
