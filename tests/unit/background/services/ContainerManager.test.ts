...
describe("ContainerManager", () => {
  ...
  describe("create", () => {
    it("should create a new container", async () => {
      ...
      ;(browser.contextualIdentities.create as jest.Mock).mockResolvedValue(
    ...
    it("should handle Firefox API errors gracefully", async () => {
      ...
      await expect(containerManager.create(request)).rejects.toThrow(
        "Firefox API error",
      )
    })
    ...
    it("should use random values when not provided", async () => {
      ...
      const createCall = (browser.contextualIdentities.create as jest.Mock).mock
        .calls[0][0]
  ...
  describe("update", () => {
    it("should update an existing container", async () => {
      ...
      mockStorageService.updateContainer = jest
        .fn()
        .mockResolvedValue(undefined)
      ;(browser.contextualIdentities.update as jest.Mock).mockResolvedValue(
        mockFirefoxContainer,
      )
  ...
  describe("syncWithFirefox", () => {
    it("should sync containers with Firefox", async () => {
      ...
      mockStorageService.getContainers = jest
        .fn()
        .mockResolvedValue(storageContainers)
    ...
    it("should handle Firefox API errors", async () => {
      ...
      await expect(containerManager.syncWithFirefox()).rejects.toThrow(
        "Firefox API error",
      )
    })
  })
  ...
  describe("cleanupTemporaryContainers", () => {
    it("should remove empty temporary containers", async () => {
      ...
      mockStorageService.getContainers = jest
        .fn()
        .mockResolvedValue([tempContainer])
      mockStorageService.removeContainer = jest
        .fn()
        .mockResolvedValue(undefined)
      mockStorageService.getRules = jest.fn().mockResolvedValue([])
    ...
    it("should handle errors when checking temporary containers", async () => {
      ...
      mockStorageService.getContainers = jest
        .fn()
        .mockResolvedValue([tempContainer])
      ;(browser.tabs.query as jest.Mock).mockRejectedValue(
  ...
  describe("event handlers", () => {
    it("should handle Firefox container creation", async () => {
      const listener = (
        browser.contextualIdentities.onCreated.addListener as jest.Mock
      ).mock.calls[0][0]
    ...
    it("should handle Firefox container removal", async () => {
      const listener = (
        browser.contextualIdentities.onRemoved.addListener as jest.Mock
      ).mock.calls[0][0]
      ...
      await listener({
        contextualIdentity: { cookieStoreId: mockContainer.cookieStoreId },
      })
    ...
    it("should handle Firefox container update", async () => {
      const listener = (
        browser.contextualIdentities.onUpdated.addListener as jest.Mock
      ).mock.calls[0][0]
  ...
  describe("handleFirefoxContainerRemoved", () => {
    ...
    it("should do nothing when container not found in storage", async () => {
      ...
      await listener({
        contextualIdentity: { cookieStoreId: "unknown-container" },
      })
  ...
  describe("utility methods", () => {
    it("should generate unique IDs", () => {
      const id1 = (
        containerManager as unknown as { generateId: () => string }
      ).generateId()
      const id2 = (
        containerManager as unknown as { generateId: () => string }
      ).generateId()
    ...
    it("should get random icon from default icons", () => {
      const icon = (
        containerManager as unknown as { getRandomIcon: () => string }
      ).getRandomIcon()

      expect(DEFAULT_CONTAINER_ICONS).toContain(
        icon as (typeof DEFAULT_CONTAINER_ICONS)[number],
      )
    })

    it("should get random color from default colors", () => {
      const color = (
        containerManager as unknown as { getRandomColor: () => string }
      ).getRandomColor()

      expect(DEFAULT_CONTAINER_COLORS).toContain(
        color as (typeof DEFAULT_CONTAINER_COLORS)[number],
      )
    })
  })
})
