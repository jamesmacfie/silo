...
describe('URLMatcher - Heroku Wildcard Scenario', () => {
  ...
  describe('Original Issue Fix', () => {
    ...
    it('should match all Heroku subdomains and paths', () => {
      ...
      herokuUrls.forEach(url => {
        expect(matcher.match(url, '*.heroku.com', MatchType.DOMAIN))
          .toBe(true, `Failed to match ${url} with *.heroku.com`);
      });
    });
    ...
    it('should NOT match non-Heroku domains', () => {
      ...
      nonHerokuUrls.forEach(url => {
        expect(matcher.match(url, '*.heroku.com', MatchType.DOMAIN))
          .toBe(false, `Incorrectly matched ${url} with *.heroku.com`);
      });
    });
  });
  ...
  describe('Real-world URL Patterns', () => {
    it('should handle common wildcard patterns correctly', () => {
      const testCases = [
        // GitHub
        { url: 'https://github.com', pattern: '*.github.com', shouldMatch: true },
        { url: 'https://www.github.com', pattern: '*.github.com', shouldMatch: true },
        { url: 'https://api.github.com', pattern: '*.github.com', shouldMatch: true },
        { url: 'https://raw.githubusercontent.com', pattern: '*.github.com', shouldMatch: false },
        
        // Google
        { url: 'https://google.com', pattern: '*.google.com', shouldMatch: true },
        { url: 'https://www.google.com', pattern: '*.google.com', shouldMatch: true },
        { url: 'https://mail.google.com', pattern: '*.google.com', shouldMatch: true },
        { url: 'https://docs.google.com', pattern: '*.google.com', shouldMatch: true },
        { url: 'https://googleapis.com', pattern: '*.google.com', shouldMatch: false },
        
        // Microsoft
        { url: 'https://microsoft.com', pattern: '*.microsoft.com', shouldMatch: true },
        { url: 'https://www.microsoft.com', pattern: '*.microsoft.com', shouldMatch: true },
        { url: 'https://office.microsoft.com', pattern: '*.microsoft.com', shouldMatch: true },
        { url: 'https://azure.microsoft.com', pattern: '*.microsoft.com', shouldMatch: true },
        
        // Amazon
        { url: 'https://amazon.com', pattern: '*.amazon.com', shouldMatch: true },
        { url: 'https://www.amazon.com', pattern: '*.amazon.com', shouldMatch: true },
        { url: 'https://aws.amazon.com', pattern: '*.amazon.com', shouldMatch: true },
        { url: 'https://amazonaws.com', pattern: '*.amazon.com', shouldMatch: false },
      ];
    ...
    it('should work with complex multi-level subdomains', () => {
      ...
      complexCases.forEach(url => {
        expect(matcher.match(url, '*.heroku.com', MatchType.DOMAIN))
          .toBe(true, `Failed to match complex subdomain ${url}`);
      });
    });
    ...
    it('should handle edge cases with ports and protocols', () => {
      ...
      edgeCases.forEach(url => {
        expect(matcher.match(url, '*.heroku.com', MatchType.DOMAIN))
          .toBe(true, `Failed to match edge case ${url}`);
      });
    });
  });
  ...
  describe('Pattern Validation', () => {
    it('should maintain security by not matching malicious patterns', () => {
      ...
      maliciousUrls.forEach(url => {
        expect(matcher.match(url, '*.heroku.com', MatchType.DOMAIN))
          .toBe(false, `Incorrectly matched potentially malicious URL ${url}`);
      });
    });
  ...
  describe('Integration with Rules Engine', () => {
    it('should work correctly when integrated with rule evaluation', () => {
      ...
      ruleScenarios.forEach(scenario => {
        const result = matcher.match(scenario.url, scenario.pattern, scenario.matchType);
