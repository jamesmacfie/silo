...
describe("URLMatcher", () => {
  ...
  describe("URL Normalization", () => {
    it("should remove default ports", () => {
      expect(matcher.normalize("https://example.com:443/")).toBe(
        "https://example.com",
      )
      expect(matcher.normalize("http://example.com:80/")).toBe(
        "http://example.com",
      )
    })

    it("should preserve non-default ports", () => {
      expect(matcher.normalize("https://example.com:8080/")).toBe(
        "https://example.com:8080/",
      )
    })

    it("should handle trailing slashes", () => {
      expect(matcher.normalize("https://example.com/")).toBe(
        "https://example.com",
      )
      expect(matcher.normalize("https://example.com/path/")).toBe(
  ...
  describe("Domain Extraction", () => {
    it("should extract domain from full URL", () => {
      expect(matcher.extractDomain("https://www.example.com/path")).toBe(
        "www.example.com",
      )
    })
  ...
  describe("Domain Matching", () => {
    it("should match exact domains", () => {
      ...
      expect(
        matcher.match("http://example.com", "example.com", MatchType.DOMAIN),
      ).toBe(true)
    })
    ...
    it("should match wildcard subdomains", () => {
      ...
      expect(
        matcher.match("https://example.com", "*.example.com", MatchType.DOMAIN),
      ).toBe(true)
    })
    ...
    describe("Wildcard Matching", () => {
      it("should match base domain when pattern has wildcard", () => {
        ...
        expect(
          matcher.match("https://github.com", "*.github.com", MatchType.DOMAIN),
        ).toBe(true)
      })
      ...
      it("should match all subdomain variants with wildcard pattern", () => {
        ...
        expect(
          matcher.match("https://example.com", pattern, MatchType.DOMAIN),
        ).toBe(true)
        expect(
          matcher.match("https://example.com/", pattern, MatchType.DOMAIN),
        ).toBe(true)
        expect(
          matcher.match("https://example.com/path", pattern, MatchType.DOMAIN),
        ).toBe(true)

        // Single level subdomains
        expect(
          matcher.match("https://www.example.com", pattern, MatchType.DOMAIN),
        ).toBe(true)
        expect(
          matcher.match("https://api.example.com", pattern, MatchType.DOMAIN),
        ).toBe(true)
        expect(
          matcher.match("https://blog.example.com", pattern, MatchType.DOMAIN),
        ).toBe(true)
      ...
      it("should not match unrelated domains with wildcard", () => {
        ...
        expect(
          matcher.match("https://example.org", pattern, MatchType.DOMAIN),
        ).toBe(false)
        expect(
          matcher.match("https://notexample.com", pattern, MatchType.DOMAIN),
        ).toBe(false)
        expect(
          matcher.match(
        ...
        expect(
          matcher.match("https://fakeexample.com", pattern, MatchType.DOMAIN),
        ).toBe(false)
      })
      ...
      it("should handle common real-world scenarios", () => {
        ...
        // Google scenarios
        expect(
          matcher.match("https://google.com", "*.google.com", MatchType.DOMAIN),
        ).toBe(true)
        expect(
          matcher.match(
      ...
      it("should work with paths in wildcard patterns", () => {
        ...
        expect(
          matcher.match(
            "https://www.heroku.com/admin/users",
            "*.heroku.com/admin",
            MatchType.DOMAIN,
          ),
        ).toBe(true)
    ...
    it("should not match different domains", () => {
      expect(
        matcher.match("https://example.org", "example.com", MatchType.DOMAIN),
      ).toBe(false)
      expect(
        matcher.match(
    ...
    describe("Edge Cases and Integration", () => {
      ...
      it("should handle query parameters and fragments with wildcards", () => {
        ...
        expect(
          matcher.match(
            "https://api.heroku.com?token=123&user=456#results",
            "*.heroku.com",
            MatchType.DOMAIN,
          ),
        ).toBe(true)
  ...
  describe("Regex Matching", () => {
    ...
    it("should handle invalid regex gracefully", () => {
      ...
      expect(
        matcher.match("https://example.com", "(invalid regex", MatchType.REGEX),
      ).toBe(false)
