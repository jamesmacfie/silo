...
function matchDomain(url: string, pattern: string): boolean {
  ...
  if (patternHostname.startsWith("*.")) {
    ...
    // Wildcard matching: *.example.com should match:
  ...
  if (patternPathname && patternPathname !== "/" && patternPathname !== "/*") {
    const normalizedPatternPath = patternPathname.endsWith("/")
      ? patternPathname
      : `${patternPathname}/`
    const normalizedUrlPath = urlPathname.endsWith("/")
      ? urlPathname
      : `${urlPathname}/`
