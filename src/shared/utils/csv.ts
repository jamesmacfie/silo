  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      ...
      if (
        ...
        (!parsed.ruleType || parsed.ruleType.toLowerCase() !== "exclude")
      ) {
        result.errors.push({
          line: lineNum,
      ...
      const rule: CreateRuleRequest = {
        ...
        metadata: {
          ...
          source:
            (options?.source as "user" | "bookmark" | "import") || "import",
          tags: [],
        },
      }
...
function shouldSkipLine(line: string): boolean {
  ...
  return (
    ...
    (lowerLine.includes("pattern,container") &&
      lowerLine.includes("match_type"))
  )
}
...
function isHeaderRow(parsed: CSVRule): boolean {
  return (
    ...
    (parsed.containerName &&
      parsed.containerName.toLowerCase() === "container_name")
  )
}
...
function parseCSVLine(line: string): CSVRule | null {
  ...
  return {
    ...
    enabled: cleanFields[5]
      ? cleanFields[5].toLowerCase() !== "false"
      : undefined,
...
export function validateCSVRow(
  ...
  if (fields.length < 2) {
    return {
      isValid: false,
      error: "Minimum 2 fields required (pattern, container)",
    }
  }
  ...
  const rule: CreateRuleRequest = {
    ...
    containerId:
      ruleType === "exclude"
        ? undefined
        : container?.cookieStoreId || undefined,
