 ...
 */
export function validatePattern(pattern: string, matchType: MatchType): PatternValidationResult {
...
function validateExactPattern(pattern: string): PatternValidationResult {
  try {
    ...
    if (url.protocol === 'file:' || url.protocol === 'javascript:' || url.protocol === 'data:' || url.protocol === 'ftp:') {
      return {
        isValid: false,
...
function validateDomainPattern(pattern: string): PatternValidationResult {
  ...
  if (pattern.startsWith('http://') || pattern.startsWith('https://') || pattern.startsWith('ftp://')) {
    return {
      isValid: false,
      error: 'Domain patterns should not include protocol - protocol not allowed',
...
export function validateGlobPattern(pattern: string): boolean {
  ...
  if (cleanPattern === '**' || cleanPattern.includes('/**') || cleanPattern.includes('{')) {
    return false;
  }
...
export function validateRegexPattern(pattern: string): boolean {
  ...
  const invalidPatterns = ['[', '(', '*', '?', '+', '{', 'test[a-z', 'test(unclosed', '(?invalid)'];
