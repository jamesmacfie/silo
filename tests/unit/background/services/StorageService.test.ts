...
describe("StorageService", () => {
  ...
  describe("Sync Storage Operations", () => {
    ...
    it("should return null for sync get failures", async () => {
      ;(browser.storage.sync.get as jest.Mock).mockRejectedValue(
        new Error("Sync not available"),
      )
    ...
    it("should throw on sync set failures", async () => {
      ;(browser.storage.sync.set as jest.Mock).mockRejectedValue(
        new Error("Sync quota exceeded"),
      )

      await expect(storageService.syncSet("syncKey", {})).rejects.toThrow(
        "Sync quota exceeded",
      )
    })
    ...
    it("should not throw on sync remove failures", async () => {
      ;(browser.storage.sync.remove as jest.Mock).mockRejectedValue(
  ...
  describe("Container Operations", () => {
    ...
    it("should reject invalid containers", async () => {
      ...
      await expect(
        storageService.setContainers([invalidContainer]),
      ).rejects.toThrow("Invalid containers")
    })
    ...
    it("should reject duplicate container IDs", async () => {
      ...
      await expect(
        storageService.setContainers([mockContainer, duplicate]),
      ).rejects.toThrow("Duplicate container ID")
    })
    ...
    it("should throw error when updating non-existent container", async () => {
      ...
      await expect(
        storageService.updateContainer("non-existent", { name: "Test" }),
  ...
  describe("Rule Operations", () => {
    ...
    it("should reject invalid rules", async () => {
      ...
      await expect(storageService.setRules([invalidRule])).rejects.toThrow(
        "Invalid rules",
      )
    })
    ...
    it("should reject duplicate rule IDs", async () => {
      ...
      await expect(
        storageService.setRules([mockRule, duplicate]),
    ...
    it("should validate INCLUDE rules require containerId", async () => {
      const includeRule = {
        ...mockRule,
        ruleType: "include" as const,
        containerId: undefined,
      }

      await expect(storageService.setRules([includeRule])).rejects.toThrow(
        "Missing containerId",
      )
    })
    ...
    it("should validate RESTRICT rules require containerId", async () => {
      const restrictRule = {
        ...mockRule,
        ruleType: "restrict" as const,
        containerId: undefined,
      }

      await expect(storageService.setRules([restrictRule])).rejects.toThrow(
        "Missing containerId",
      )
    })
    ...
    it("should warn about containerId on EXCLUDE rules", async () => {
      ...
      await expect(
        storageService.setRules([excludeRule]),
    ...
    it("should validate regex patterns", async () => {
      ...
      await expect(storageService.setRules([regexRule])).rejects.toThrow(
    ...
    it("should resort rules when priority changes", async () => {
      ...
      ;(browser.storage.local.get as jest.Mock).mockResolvedValue({
        [STORAGE_KEYS.RULES]: [rule1, rule2],
      })
    ...
    it("should throw error when updating non-existent rule", async () => {
      ...
      await expect(
        storageService.updateRule("non-existent", { priority: 5 }),
      ).rejects.toThrow("Rule not found: non-existent")
    })
  ...
  describe("Preferences Operations", () => {
    ...
    it("should merge saved preferences with defaults", async () => {
      ...
      ;(browser.storage.local.get as jest.Mock).mockResolvedValue({
        [STORAGE_KEYS.PREFERENCES]: savedPrefs,
      })
    ...
    it("should update preferences", async () => {
      ...
      ;(browser.storage.local.get as jest.Mock).mockResolvedValue({
        [STORAGE_KEYS.PREFERENCES]: currentPrefs,
      })
  ...
  describe("Bookmark Associations", () => {
    ...
    it("should update existing bookmark association", async () => {
      ...
      ;(browser.storage.local.get as jest.Mock).mockResolvedValue({
        [STORAGE_KEYS.BOOKMARKS]: [existingBookmark],
      })
    ...
    it("should get specific bookmark association", async () => {
      ...
      const result = await storageService.getBookmarkAssociation(
        mockBookmark.bookmarkId,
      )
  ...
  describe("Category Operations", () => {
    ...
    it("should rename category and update container references", async () => {
      const categories = ["Work", "Personal"]
      const containerWithCategory = {
        ...mockContainer,
        metadata: { ...mockContainer.metadata, categories: ["Work"] },
      }

      ;(browser.storage.local.get as jest.Mock)
        .mockResolvedValueOnce({ [STORAGE_KEYS.CATEGORIES]: categories })
        .mockResolvedValueOnce({
          [STORAGE_KEYS.CONTAINERS]: [containerWithCategory],
        })
  ...
  describe("Stats Operations", () => {
    ...
    it("should record open stat", async () => {
      ;(browser.storage.local.get as jest.Mock)
        .mockResolvedValueOnce({
          [STORAGE_KEYS.PREFERENCES]: { stats: { enabled: true } },
        })
    ...
    it("should record close stat", async () => {
      ...
      ;(browser.storage.local.get as jest.Mock)
        ...
        .mockResolvedValueOnce({
          [STORAGE_KEYS.STATS]: { "firefox-container-1": existingStats },
        })
    ...
    it("should trim history when it exceeds 1000 entries", async () => {
      const largeHistory = Array.from({ length: 1001 }, (_, i) => ({
        timestamp: i,
        event: "open" as const,
      }))
  ...
  describe("Template Operations", () => {
    ...
    it("should update existing template", async () => {
      ...
      ;(browser.storage.local.get as jest.Mock).mockResolvedValue({
        [STORAGE_KEYS.TEMPLATES]: [existingTemplate],
      })
  ...
  describe("Migration and Setup", () => {
    it("should handle fresh install", async () => {
      ...
      const setupCall = calls.find(
        (call) => call[0][STORAGE_KEYS.CONTAINERS] !== undefined,
      )
      expect(setupCall).toBeDefined()
      expect(setupCall[0]).toEqual(
        expect.objectContaining({
          ...
          [STORAGE_KEYS.CATEGORIES]: expect.arrayContaining([
            "Work",
            "Personal",
          ]),
  ...
  describe("Backup and Restore", () => {
    it("should create backup", async () => {
      ...
      ;(browser.storage.local.get as jest.Mock)
        .mockResolvedValueOnce({
          [STORAGE_KEYS.CONTAINERS]: mockData.containers,
        })
        .mockResolvedValueOnce({ [STORAGE_KEYS.RULES]: mockData.rules })
        .mockResolvedValueOnce({
          [STORAGE_KEYS.PREFERENCES]: mockData.preferences,
        })
        .mockResolvedValueOnce({ [STORAGE_KEYS.BOOKMARKS]: mockData.bookmarks })
        .mockResolvedValueOnce({
          [STORAGE_KEYS.CATEGORIES]: mockData.categories,
        })
    ...
    it("should handle missing data in backup", async () => {
      ;(browser.storage.local.get as jest.Mock).mockResolvedValue({}) // All get calls return empty
    ...
    it("should restore from backup", async () => {
      ...
      expect(browser.storage.local.set).toHaveBeenCalledWith({
        [STORAGE_KEYS.CONTAINERS]: backup.containers,
      })
      expect(browser.storage.local.set).toHaveBeenCalledWith({
        [STORAGE_KEYS.RULES]: backup.rules,
      })
      expect(browser.storage.local.set).toHaveBeenCalledWith({
        [STORAGE_KEYS.PREFERENCES]: backup.preferences,
      })
      expect(browser.storage.local.set).toHaveBeenCalledWith({
        [STORAGE_KEYS.BOOKMARKS]: backup.bookmarks,
      })
      expect(browser.storage.local.set).toHaveBeenCalledWith({
        [STORAGE_KEYS.CATEGORIES]: backup.categories,
      })
      expect(browser.storage.local.set).toHaveBeenCalledWith({
        [STORAGE_KEYS.STATS]: backup.stats,
      })
    })
