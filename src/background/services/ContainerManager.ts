...
export class ContainerManager {
  ...
  private setupEventListeners(): void {
    ...
    if (browser.contextualIdentities?.onCreated) {
      ...
      browser.contextualIdentities.onRemoved.addListener(async (changeInfo) => {
        ...
        await this.handleFirefoxContainerRemoved(changeInfo.contextualIdentity.cookieStoreId);
      });
  ...
  async delete(id: string): Promise<void> {
    ...
    try {
      ...
      this.log.warn('Failed to remove Firefox container (may already be deleted)', error);
    }
    ...
    const filteredRules = rules.filter(rule => rule.containerId !== container.cookieStoreId);
    ...
    this.log.info('Container deleted successfully', { id: container.id, cookieStoreId: container.cookieStoreId });
  }
  ...
  async syncWithFirefox(): Promise<void> {
    ...
    try {
      ...
      for (const ffContainer of firefoxContainers) {
        const exists = ourContainers.find(c => c.cookieStoreId === ffContainer.cookieStoreId);
  ...
  async mapToFirefoxContainer(container: Container): Promise<string> {
    ...
    try {
      const firefoxContainer = await browser.contextualIdentities.get(container.cookieStoreId);
  ...
  private async handleFirefoxContainerRemoved(cookieStoreId: string): Promise<void> {
    const containers = await this.storage.getContainers();
  ...
  private async cleanupTemporaryContainers(): Promise<void> {
    ...
    const containers = await this.storage.getContainers();
    const temporaryContainers = containers.filter(c => c.temporary || c.metadata?.lifetime === 'untilLastTab');
    ...
    for (const container of temporaryContainers) {
      try {
        ...
        this.log.error('Error checking temporary container', { container, error });
      }
    }
  }
