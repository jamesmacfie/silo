...
describe("BookmarkIntegration", () => {
  ...
  beforeEach(() => {
    ...
    mockStorageService.addBookmarkAssociation = jest
      .fn()
      .mockResolvedValue(undefined)
    mockStorageService.removeBookmarkAssociation = jest
      .fn()
      .mockResolvedValue(undefined)
    mockStorageService.getBookmarkAssociation = jest
      .fn()
      .mockResolvedValue(null)
    mockStorageService.getBookmarkAssociations = jest.fn().mockResolvedValue([])
    mockStorageService.setBookmarkAssociations = jest
      .fn()
      .mockResolvedValue(undefined)
    ...
    bookmarkIntegration = new BookmarkIntegration()
  })
  ...
  describe("associate", () => {
    it("should create bookmark association with all parameters", async () => {
      ...
      await bookmarkIntegration.associate(
        bookmarkId,
        containerId,
        url,
        autoOpen,
  ...
  describe("disassociate", () => {
    it("should remove bookmark association", async () => {
      ...
      expect(mockStorageService.removeBookmarkAssociation).toHaveBeenCalledWith(
        bookmarkId,
      )
    })
  })
  ...
  describe("getAssociation", () => {
    it("should retrieve specific bookmark association", async () => {
      ...
      mockStorageService.getBookmarkAssociation = jest
        .fn()
        .mockResolvedValue(mockBookmarkAssociation)

      const result = await bookmarkIntegration.getAssociation(bookmarkId)

      expect(result).toEqual(mockBookmarkAssociation)
      expect(mockStorageService.getBookmarkAssociation).toHaveBeenCalledWith(
        bookmarkId,
      )
    })
  ...
  describe("getAllAssociations", () => {
    it("should retrieve all bookmark associations", async () => {
      ...
      mockStorageService.getBookmarkAssociations = jest
        .fn()
  ...
  describe("syncBookmarks", () => {
    ...
    it("should handle browser API errors gracefully", async () => {
      ;(browser.bookmarks.getTree as jest.Mock).mockRejectedValue(
        new Error("Bookmarks API unavailable"),
      )
    ...
    it("should resolve container hints to cookieStoreIds", async () => {
      ...
      mockContainerManager.getAll = jest
        .fn()
        .mockResolvedValue([mockContainer, personalContainer])
  ...
  describe("resolveToCookieStoreId (private)", () => {
    ...
    beforeEach(() => {
      mockContainerManager.getAll = jest
        .fn()
        .mockResolvedValue(containers as Container[])
    })
    ...
    it("should be case insensitive", async () => {
      const result1 = await (bookmarkIntegration as any).resolveToCookieStoreId(
        "PERSONAL",
      )
      const result2 = await (bookmarkIntegration as any).resolveToCookieStoreId(
        "work container",
      )
      const result3 = await (bookmarkIntegration as any).resolveToCookieStoreId(
        "FIREFOX-CONTAINER-2",
      )
    ...
    it("should prioritize cookieStoreId over id over name", async () => {
      ...
      mockContainerManager.getAll = jest
        .fn()
        .mockResolvedValue(ambiguousContainers as Container[])
